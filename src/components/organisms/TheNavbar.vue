...
<template>
  <!-- relative est nécessaire pour que le tiroir mobile (position:absolute) soit ancré au header -->
  <header class="relative sticky top-0 z-50 bg-surface/80 backdrop-blur-lg shadow-sm">
    ...
    >
      ...
      <!-- ─── Desktop : sélecteur de langue + bouton CTA ─── -->
      <div class="hidden md:flex items-center gap-4">
        ...
        >
          <button
            v-for="lang in ['fr', 'nl']"
            :key="lang"
            class="px-3 min-h-[36px] text-xs font-semibold font-manrope uppercase transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
            :class="
              locale === lang
      ...
      <!-- ─── Mobile : bouton hamburger (touch target min 48×48 px) ─── -->
      <button
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
          ...
          >
            <button
              v-for="lang in ['fr', 'nl']"
              :key="lang"
              class="px-5 min-h-[48px] text-sm font-semibold font-manrope uppercase rounded-xl transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              :class="
                locale === lang
                  ? 'bg-primary text-white'
